...
async def run_command(command, directory):
    logging.info(f"Executing: {command} (in {directory})")
    ...
    async for line in process.stdout:
        logging.info(f"{directory}: {line.decode().strip()}")

    async for line in process.stderr:
        logging.error(f"{directory}: {line.decode().strip()}")
...
async def copy_plan_file(directory, plan_output_file):
    ...
    try:
        for destination in [project_plan_path, current_plan_path]:
            ...
            logging.info(f"Terraform plan file copied to {destination}")
    except Exception as e:
        logging.error(f"Error copying Terraform plan file in {directory}: {str(e)}")

async def run_terraform_plan(directory, config):
    if not os.path.exists(os.path.join(directory, "main.tf")):
        logging.warning(f"Skipping {directory}: No Terraform files found.")
        return

    logging.info(f"Running 'terraform plan' in {directory} using workspace '{config['workspace']}'...")
    ...
    try:
        ...
        logging.info(f"SUCCESS: Terraform plan completed in {directory}")
    except Exception as e:
        logging.error(f"Failed to run Terraform in {directory}: {str(e)}")